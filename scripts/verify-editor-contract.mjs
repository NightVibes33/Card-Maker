...
requireMatch(
  aiCutout,
  /if \(!isAppleMobileBrowser\(currentNavigator\)\)[\s\S]{0,360}requestAdapter\([\s\S]{0,180}const devices = canUseWebGPU \? \['webgpu', 'wasm'\] : \['wasm'\]/,
  'iOS cutouts skip unsupported WebGPU and use WASM while supported desktop browsers retain WebGPU'
);
requireMatch(
  aiCutout,
  /env\.backends\.onnx\.wasm\.numThreads = 1/,
  'iOS cutouts explicitly use single-threaded WASM'
);
requireMatch(
  aiCutout,
  /dtype: device === 'webgpu' \? 'fp16' : 'q8'/,
  'AI cutouts select matching precision for WebGPU and WASM'
);
