...
const pageChecks = [
  ...
  [/function rememberDecodedCatalogImage\(/, 'decoded Library thumbnails are retained for instant editor handoff'],
  [/backgroundPreviewImage: decodedPreview/, 'Library selection hands the already-visible decoded image into Studio'],
  [/if \(previewImage\) \{[\s\S]{0,520}setLoadedBackgroundKey\(backgroundKey\)/, 'Studio keeps the decoded Library preview renderable while high resolution loads'],
