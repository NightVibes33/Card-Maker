          ...
          <div className="tabScreen studioScreen" role="tabpanel" id="panel-studio" aria-labelledby="tab-studio">
            ...
            {studioTool === 'background' ? (
              <section className="studioContextCard capcutContextPanel">
                ...
                {studioSubtool==='image'?<><button type="button" className="capcutPrimaryTile" disabled={imageImportInProgress || presetTransferInProgress || cleanupInProgress} onClick={()=>{uploadIntentRef.current='replace-artwork';uploadRef.current?.click()}}><IOSIcon name="photo" size={20}/> Import Photo or File</button><div className="backgroundRecentRail">{recent.slice(0,8).map((item,i)=><button type="button" key={item.id||item.image||i} onClick={()=>item.image&&useArtwork(item)}>{item.image?<img src={item.image} alt=""/>:<span>Image</span>}</button>)}</div></>:null}
